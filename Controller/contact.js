...
export const addContact = async (req, res) => {
  ...
  try {
    if (!firstName || !lastName || !email ||  !message) {
      return res.status(400).json({ success: false, message: "All fields (firstName, lastName, email,  message) are required" });
    }
    ...
    const result = await contactModal.create({ name, email1:email,    message });
